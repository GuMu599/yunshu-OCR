---
name: yunshu-ocr
description: Use when Claude Code receives a PDF attachment or path and must read, summarize, search, compare, quote, verify, or answer questions about PDF content.
---

# Yunshu-OCR for Claude Code

## Core contract

The user works with the original PDF. Claude Code reads the bound Markdown first. Never
...
Resolve the local PDF path supplied to Claude Code, then run the launcher beside this
`SKILL.md`:
...
layout exists, use `render-page` or Claude Code's available PDF/image reading capability.
